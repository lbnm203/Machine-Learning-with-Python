...
# Hàm train KMeans
# def train_kmeans(X_train, n_clusters, init, max_iter):
...
# # Hàm visualize kết quả phân cụm
...
#     fig, ax = plt.subplots()
#     scatter = ax.scatter(
#         X_reduced[:, 0], X_reduced[:, 1], c=labels, cmap='viridis', alpha=0.5)
#     ax.set_title(title)
...
#     # Chọn số lượng ảnh để train
#     num_samples = st.slider("Số lượng ảnh để training",
...
#     # Chọn tỷ lệ train và test
#     train_size = st.slider("Tỷ lệ train", 0.1, 0.9, 0.5)
#     test_size = st.slider("Tỷ lệ test", 0.1, 0.5, 0.2)
#     val_size = 1.0 - (train_size + test_size)

#     # Kiểm tra nếu tổng tỷ lệ > 1.0
#     if val_size < 0:
#         st.error("Tổng tỷ lệ train + test phải nhỏ hơn hoặc bằng 1.0! Hãy chọn lại.")
#         st.stop()

#     # Chia tập train và test trước
#     X_train, X_temp, y_train, y_temp = train_test_split(
#         X, y, train_size=train_size, random_state=42, stratify=y)

#     # Phần còn lại là validation
...
#         "Tỷ lệ (%)": [int(train_size * 100), int(val_size * 100), int(test_size * 100)]
#     }
#     st.table(data)

#     st.write("---")
#     # Chọn mô hình
#     model_type = st.selectbox("Chọn mô hình phân cụm:", ["KMeans", "DBSCAN"])
...
#         if st.button("Huấn luyện KMeans"):
#             model = train_kmeans(X_train, n_clusters, init, max_iter)
#             st.session_state["kmeans_model"] = model  # Lưu mô hình vào session
#             labels_val = model.predict(X_val)
#             score = silhouette_score(X_val, labels_val)
#             st.write(f"Silhouette Score trên tập validation: {score:.4f}")
#             plot_clusters(X_val, labels_val, "KMeans Clustering")

#     elif model_type == "DBSCAN":
#         eps = st.slider("Epsilon (eps)", 0.1, 5.0, 1.0)
#         min_samples = st.slider("Số điểm tối thiểu", 2, 20, 5)

#         if st.button("Huấn luyện DBSCAN"):
#             model = train_dbscan(X_train, eps, min_samples)
#             st.session_state["dbscan_model"] = model  # Lưu mô hình vào session
#             labels_val = model.fit_predict(X_val)
#             score = silhouette_score(X_val, labels_val) if len(
#                 set(labels_val)) > 1 else -1
#             st.write(f"Silhouette Score trên tập validation: {score:.4f}")
#             plot_clusters(X_val, labels_val, "DBSCAN Clustering")
